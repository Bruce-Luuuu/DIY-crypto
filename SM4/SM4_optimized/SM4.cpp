...
#define MM256_PACK1_EPI32(a, b, c, d) _mm256_unpackhi_epi64(_mm256_unpacklo_epi32(a, b), _mm256_unpacklo_epi32(c, d))
#define MM256_PACK2_EPI32(a, b, c, d) _mm256_unpacklo_epi64(_mm256_unpackhi_epi32(a, b), _mm256_unpackhi_epi32(c, d))
#define MM256_PACK3_EPI32(a, b, c, d) _mm256_unpackhi_epi64(_mm256_unpackhi_epi32(a, b), _mm256_unpackhi_epi32(c, d))
...
// 构建 T-table 查表优化 S盒和L变换
const uint32_t T_table0[256] = {
...
void SM4_KeyInit(uint8_t* key, SM4_Key sm4_key) {
    ...
    for (int i = 0; i < 4; i++) {
        int j = 4 * i;
        MK[i] = (key[j] << 24) | (key[j + 1] << 16) | (key[j + 2] << 8) | (key[j + 3]);
        MK[i] = MK[i] ^ FK[i];
    }
    // 32轮迭代
    for (int i = 0; i < 32; i++) {
        tmp = MK[1] ^ MK[2] ^ MK[3] ^ CK[i];
        // S盒（强制转化成1字节过S）
        for (int j = 0; j < 4; j++) {
            ((uint8_t*)&tmp)[j] = SBox[((uint8_t*)&tmp)[j]];
        }
        // 线性变换 L
        sm4_key[i] = MK[0] ^ tmp ^ rotl32(tmp, 13) ^ rotl32(tmp, 23);
        // 移位
        MK[0] = MK[1]; MK[1] = MK[2]; MK[2] = MK[3]; MK[3] = sm4_key[i];
    }
    return;
}
...
void _SM4_do(uint8_t* in, uint8_t* out, SM4_Key sm4_key, int enc) {
    ...
    X[3] = _mm256_shuffle_epi8(X[3], vindex);
    //恢复分组并装填
    _mm256_storeu_si256((__m256i*)out + 0, MM256_PACK0_EPI32(X[3], X[2], X[1], X[0]));
